...
async def send_in_background(
    ...
    return JSONResponse(status_code=200, content={"message": "telegram chat has been send "})
...
async def fetch_daily_energies():
    ...
    for _, energy in energy_mapping.items():
        if not previous_energy:
            ...
            daily_energy.append({'id': energy['id'], 'created_at': energy['created_at'], 'energy': 0, 'power': energy['power'], 'voltage': energy['voltage'], 'current': energy['current'], 'frequency': energy['frequency'], 'power_factor': energy['power_factor']})
            continue
...
@app.get("/predict")
async def fetch_nextmonth_predict():
    nextmonth = [
        {
            "month_simple": "Sept",
            "month_full": "September",
            "energy_estimated": 600
        }
    ]
    return nextmonth
